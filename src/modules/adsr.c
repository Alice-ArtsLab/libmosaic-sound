...
void mscsound_process_silence(mscsound_adsr_t **adsr) {
  *((*adsr)->output0) = *((*adsr)->input0);
  ...
  if (!(*(*adsr)->play)) {
    ...
    (*adsr)->finalSample =
        (((*adsr)->sampleRate * *((*adsr)->attack)) / 1000) - 1;
    (*adsr)->process(adsr);
  }
}
...
void mscsound_process_attack(mscsound_adsr_t **adsr) {
  *((*adsr)->output0) = *((*adsr)->input0);
  ...
  float amplitudeStep =
      1 / (float)(((*adsr)->finalSample) - (*adsr)->initialSample);
  ...
  if (remainingSamples > (*adsr)->framesPerBuffer) {
    ...
    (*adsr)->finalSample +=
        (((*adsr)->sampleRate * *((*adsr)->decay)) / 1000) - 1;
    if (remainingSamples != (*adsr)->framesPerBuffer)
      (*adsr)->process(adsr);
  }
  ((*adsr)->currentFrame) = 0;
...
void mscsound_process_decay(mscsound_adsr_t **adsr) {
  *((*adsr)->output0) = *((*adsr)->input0);
  ...
  if (remainingSamples > (*adsr)->framesPerBuffer) {
    ...
    (*adsr)->initialSample = (*adsr)->currentSample;
    (*adsr)->finalSample +=
        (((*adsr)->sampleRate * *((*adsr)->sustain)) / 1000) - 1;
    if (remainingSamples != (*adsr)->framesPerBuffer)
      (*adsr)->process(adsr);
  }
  ((*adsr)->currentFrame) = 0;
...
void mscsound_process_sustain(mscsound_adsr_t **adsr) {
  *((*adsr)->output0) = *((*adsr)->input0);
  ...
  if (remainingSamples > (*adsr)->framesPerBuffer) {
    ...
    (*adsr)->initialSample = (*adsr)->currentSample;
    (*adsr)->finalSample +=
        (((*adsr)->sampleRate * *((*adsr)->release)) / 1000) - 1;
    if (remainingSamples != (*adsr)->framesPerBuffer)
      (*adsr)->process(adsr);
  }
  ((*adsr)->currentFrame) = 0;
